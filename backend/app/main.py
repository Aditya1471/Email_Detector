...
app = FastAPI(title="PhishGuard API Gateway", description="Explainable Phishing and Fraud Email Detection Service Backend", version="1.2.0")
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    return JSONResponse(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, content={"detail": jsonable_encoder(exc.errors())})
...
async def unhandled_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"detail": "An unexpected server error occurred.", "request_id": request_id})
...
def get_health_status():
    ...
    return {"status": "healthy", "service": "phishguard-api-gateway", "version": "1.2.0", "database": database_status}


if __name__ == "__main__":
    uvicorn.run("main:app", host=settings.APP_HOST, port=settings.APP_PORT, reload=True if settings.APP_ENV == "development" else False)
