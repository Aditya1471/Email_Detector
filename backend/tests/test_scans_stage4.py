...
def test_scan_delete_cascade_feedback(db_session, test_user_a, token_a):
    ...
    feedback_payload = {
        "rating": "yes",
        "comment": "Accurate prediction feedback"
    }
    fb_resp = client.post(
        f"/api/v1/scans/{scan.id}/feedback",
...
def test_duplicate_feedback_prevention(db_session, test_user_a, token_a):
    ...
    feedback_payload = {
        "rating": "yes",
        "comment": "Accurate feedback 1"
    }
    # Submit first
    fb_resp1 = client.post(
        f"/api/v1/scans/{scan.id}/feedback",
        json=feedback_payload,
        headers={"Authorization": f"Bearer {token_a}"}
    )
...
def test_dashboard_stats_calculations(db_session, test_user_a, token_a):
    ...
    scan1 = Scan(
        user_id=test_user_a.id, classification="safe", risk_score=10, confidence=0.9,
        legitimate_probability=0.9, phishing_probability=0.1, model_version="test",
        processing_time_ms=50, url_count=0, indicator_count=0, high_severity_count=0,
        sender_domain="a.com", recipient_domain="a.com", subject_preview="S1",
        indicators_json=[], urls_json=[]
    )
    scan2 = Scan(
        user_id=test_user_a.id, classification="safe", risk_score=20, confidence=0.8,
        legitimate_probability=0.8, phishing_probability=0.2, model_version="test",
        processing_time_ms=50, url_count=0, indicator_count=0, high_severity_count=0,
        sender_domain="a.com", recipient_domain="a.com", subject_preview="S2",
        indicators_json=[], urls_json=[]
    )
    scan3 = Scan(
        user_id=test_user_a.id, classification="phishing", risk_score=90, confidence=0.95,
        legitimate_probability=0.05, phishing_probability=0.95, model_version="test",
        processing_time_ms=50, url_count=0, indicator_count=0, high_severity_count=0,
        sender_domain="a.com", recipient_domain="a.com", subject_preview="S3",
        indicators_json=[], urls_json=[]
    )
